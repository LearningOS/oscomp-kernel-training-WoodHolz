...
$I/initcode:
	$(CC) $(CFLAGS) -c -o $I/initcode.o $I/initcode.S
$K/initcode: $I/initcode
	cp $I/initcode.o $K/initcode.o

# Compile Kernel
kernel-qemu: $(OBJS) $(linker) $U/initcode $K/initcode
...
all: $U/initcode $K/initcode kernel-qemu 
...
test: test.S
	$(CC) $(CFLAGS) -march=rv64g -nostdinc -I. -Ikernel -c test.S -o test.o
	$(LD) $(LDFLAGS) -N -e start -Ttext 0 -o test.out test.o
	$(OBJCOPY) -S -O binary test.out test
	$(OBJDUMP) -S test.o > test.asm
...
clean: 
	rm -f *.tex *.dvi *.idx *.aux *.log *.ind *.ilg *.asm *.sym \
	*/*.o */*.d */*.asm */*.sym \
	$T/* \
