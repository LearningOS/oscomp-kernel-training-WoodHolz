...
userinit(void)
{
  ...
  // uvminit(p->pagetable , p->kpagetable, initcod, sizeof(initcod));
  p->sz = PGSIZE;
...
int do_clone(uint64 stack, uint64 epc){
  //uint64 epc;
  //fetchaddr(stack, &epc);  
  if(stack == 0) 
    return fork();
...
reparent(struct proc *p)
{
  ...
  for(pp = proc; pp < &proc[NPROC]; pp++){
    ...
    // if pp or a child of pp were also in ()
...
int wait4(int pid, int *status, int options)
{
  ...
  for(;;){
    ...
    for(child = proc; child < &proc[NPROC]; child++){
      ...
      acquire(&child->lock); // confused

      if(child->state == ZOMBIE){
        // Found one.
        foundpid = child->pid;
        *status = child->xstate << 8;
        freeproc(child);
        release(&child->lock);
        release(&p->lock);
        return foundpid;
