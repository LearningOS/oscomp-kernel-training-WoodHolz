...
//fat与ext4
int ls(struct file *f, uint64 addr, int size)
{

    struct dirent *ep = f->ep; 

    struct dirent ep2;
    struct ext4_dirent64 dt;
    struct ext4_dirent64 *dt2 = NULL;;
    ...
    while ((type = enext(ep, &ep2, off, &count)) != -1)
    {
        ...
        dt2 = (struct ext4_dirent64 *)addr;
        copyout2((uint64)dt2->d_name, ep2.filename, strlen(ep2.filename)+1);
        //printf("sizeofdt%d\n", sizeof(dt));
